...
`
	...
	installList := dependencies.InstallList("")
	if len(installList) != 2 {
		t.Error("Expected to have an install list with two elements")
	}
}

func TestInstallListWithRepo(t *testing.T) {
	setupTestPwd()
	setupEnv()
...
`
	...
	installList := dependencies.InstallList("github.com/d2fb/gopack")
	if len(installList) != 2 {
		t.Error("Expected to have an install list with two elements")
	}
}
